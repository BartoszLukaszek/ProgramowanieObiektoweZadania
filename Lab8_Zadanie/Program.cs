...
class Program
{
        ...
        IEnumerable<Country> countries)
    {
        var result = from c in cities
                     group c by c.CountryCode into countrys
                     select (countrys.Key, countrys.Count());
        return result;
    }
    ...
    {
        var x = countries.GroupJoin(cities, country => country.Capital, city => city.Name, (country, city) => (country, city)).Where(x=>x.country.Capital.Any() && x.country.Capital.Length > 0 && x.city.Count() > 0 ).Select(x=>(x.country.CountryName,x.country.Capital,x.city.FirstOrDefault().Population)).OrderBy(x=>x.CountryName);
    ...
    public static IEnumerable<string> RandomNames(int count)
    {
        if(count>2)
        {
            string[] name = new string[3]{"Ewa","Karol","Adam"};
    ...
    public static IEnumerable<string> Tree(int height)
    {
        ...
        var f = Enumerable.Range(0, height).Select(i=> starts.Append("*"));
