...
class App
{
    public static void Main(string[] args)
    {
        ...
        Musician<Instrument> musician = new Musician<Instrument>() { Instrument = new Keyboard() };
        Console.WriteLine(musician.ToString());
        Musician<Instrument> musician2 = new Musician<Instrument>() { Instrument = new Guitar() };
        Console.WriteLine(musician2.ToString());
        Musician<Instrument> musician3 = new Musician<Instrument>() { Instrument = new Drum() };
        Console.WriteLine(musician3.ToString());
        Console.WriteLine();
        Console.WriteLine("ćwiczenie 2");
        var tuple1 = Exercise2.getTuple1();
        int[] arr = { 1, 2, 3, 5, 5 };
        var tuple = Exercise2.GetTuple2(arr);
...
public class Exercise2
{
    ...
    public static ValueTuple<T[], bool> GetTuple2<T>(T[] arr)
    {
        if (arr.Length > 0 )
        {
            T[] localArr = arr;
            localArr[0] = arr[0];
            localArr[1] = arr[arr.Length - 1];
            //??
            return (firstAndLast: localArr, isSame: true);
        }
        else
        {
            T[] localArr = { };
            return (firstAndLast: localArr, isSame: false);
